...
module.exports = {
    ...
    components: {
      ...
      return {
        ...
        default_value: {},
        rules: {
            std_symbol:[{required: true, message: '此处不能为空', trigger: 'blur'}],
            lp_symbol:[{required: true, message: '此处不能为空', trigger: 'blur'}],
            weight:[
                {required: true, message: '此处不能为空', trigger: 'blur'},
                {
                    trigger: 'blur',
                    validator:(rule, value, callback)=>{
                            var val = this.default_value.weight;
                            console.log('val',val);
                           if(Number(val) != val){
             ...
             min_qty:[
                {required: true, message: '此处不能为空', trigger: 'blur'},
                {
                    trigger: 'blur',
                    validator:(rule, value, callback)=>{
                            var val = this.default_value.min_qty;
                            console.log('val',val);
                           if(Number(val) != val){
                                    callback(new Error('这里请填写数字类型'));
                            }else if(Number(val)<0){
            ...
            contract_size:[
                {required: true, message: '此处不能为空', trigger: 'blur'},
                {
                    trigger: 'blur',
                    validator:(rule, value, callback)=>{
                            var val = this.default_value.contract_size;
                            console.log('val',val);
                           if(Number(val) != val){
                                    callback(new Error('这里请填写数字类型'));
                            }else if(Number(val)<0){
                ...
                if(item.key == 'lp'){
                    if(item.type == 'select'){
                      ...
                      this.$store.state.global.lps.forEach((lp,index)=>{
                            this.$set(item.value.list, index, {value: lp,text:lp} );
                      })
                    }else{
                      item.hidden = true;
                    }
